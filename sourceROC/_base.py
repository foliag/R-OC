...
class SGLBaseEstimator(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        if is_regressor(self) and self.loss.lower() not in allowed_losses:
            raise ValueError(
                "For regression, the argument loss must be one of {0};"
                "got {1}".format(allowed_losses, self.loss)
            )
        ...
        if self.loss == "huber":
            f = cp.utils.HuberLoss(X, y, delta = self.delta)
        ...
        groups = check_groups(
            self.groups, X, allow_overlap=True, fit_intercept=self.fit_intercept
        )
